...
for i in range(len(X_train_filenames)):  #len(X_train_filenames)
    # try:
    if i%100==0:
    ...
    if True:
        ...
        Filesave = '/mnt/sda1/downloads/cocoapi-master/PythonAPI/resized128_rot_test11'+X_train_filenames[i][23:]
        # FileName = batch_x[i]
        ...
        while c<6:
            ...
            if sum(sum(labels_conv>1))==0 and sum(sum(labels_conv<0.01))==0:
                c=11
...
np.save('pre_calc/test1_rot_all_labels.npy',all_labels)
np.save('pre_calc/test1_rot_all_coord.npy',all_coord)
np.save('pre_calc/test1_rot_new_names.npy',X_train_filenames)
