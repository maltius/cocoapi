...
path='/datadrive1/pre_processed_HM/files_aic_17_cropped_rot_face_11_nz/'

face_labels=np.load('/mnt/sda1/downloads/BlazePose-tensorflow-master/data_configs_aic/rotation_face_labels.npy')
body_labels=np.load('/mnt/sda1/downloads/BlazePose-tensorflow-master/data_configs_aic/rotation_body_labels.npy')
...
X_train_filenames21=np.load('/mnt/sda1/downloads/BlazePose-tensorflow-master/data_configs_aic/rotation_new_names.npy')
...
if True:
    for idx in range(int(filenames.shape[0]/128)):
        ...
        if train_mode==0:
            heatmap_set = np.zeros((tot_data, 128, 128, 11), dtype=np.float32)
        ...
        if reduce_it==1:
            ...
            if train_mode==0:
                heatmap_set1 = np.zeros((tot_data-len(to_be_omitted), 128, 128, 11), dtype=np.float32)
