...
# path='/mnt/sda1/downloads/dataset/data/imagess/'
path='/mnt/sda1/downloads/BlazePose-tensorflow-master/temp/body/'

path1='/mnt/sda1/downloads/dataset/data/res/'

jfiles1 = [f for f in os.listdir(path) if f.endswith('.jpg')]

jfiles=sorted(jfiles1)

files = glob.glob('/mnt/sda1/downloads/dataset/data/res/*.jpg')
for f in files:
    os.remove(f)
...
for i in range(len(jfiles)):
    try:
        ...
        if min(ii.shape[0:2])>30:
            ...
            train_data[c] = tf.image.resize(img, [256, 256])
...
for t in range(c):
    ...
    for i in range(keyp):
        if i==6 or i==9:
            ...
            # cv2.putText(img, str(i), tuple(skeleton[i][0:2]), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (0,0,255))
    cv2.imwrite(path1+"test"+str(t).zfill(5)+".jpg", img)
